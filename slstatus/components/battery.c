...
const char *battery_state(const char *bat)
{
    ...
    if (esnprintf(path, sizeof(path), "/sys/class/power_supply/%s/status", bat) < 0) {
        return NULL;
    }
    if (pscanf(path, "%12[^\n]", state) != 1) {
        return NULL;
...
const char *battery_remaining(const char *bat)
{
    ...
    if (!pick(bat, "/sys/class/power_supply/%s/charge_now", "/sys/class/power_supply/%s/energy_now",
              path, sizeof(path))
...
const char *battery_remaining(const char *unused)
{
    ...
    if (load_apm_power_info(&apm_info)) {
        if (apm_info.ac_state != APM_AC_ON) {
            return bprintf("%uh %02um", apm_info.minutes_left / 60, apm_info.minutes_left % 60);
        } else {
            return "";
        }
    }
