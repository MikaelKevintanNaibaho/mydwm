...
static void bstack(Monitor *m)
{
    ...
    for (i = 0, c = nexttiled(m->clients); c; c = nexttiled(c->next), i++) {
        if (i < m->nmaster) {
            ...
            resize(c, sx, sy,
                   sw * (c->cfact / sfacts) + ((i - m->nmaster) < srest ? 1 : 0) - (2 * c->bw),
                   sh - (2 * c->bw), 0);
            sx += WIDTH(c) + iv;
        }
    }
        ...
        if (!m->nmaster || i < m->nmaster) {
            ...
            if ((i - m->nmaster) % 2) {
                resize(c, lx, ly, lw - (2 * c->bw),
                       lh * (c->cfact / lfacts) + ((i - 2 * m->nmaster) < 2 * lrest ? 1 : 0)
                           - (2 * c->bw),
                       0);
                ly += HEIGHT(c) + ih;
            } else {
                resize(c, rx, ry, rw - (2 * c->bw),
                       rh * (c->cfact / rfacts) + ((i - 2 * m->nmaster) < 2 * rrest ? 1 : 0)
                           - (2 * c->bw),
                       0);
                ry += HEIGHT(c) + ih;
            }
...
void horizgrid(Monitor *m)
{
    ...
    for (i = 0, c = nexttiled(m->clients); c; c = nexttiled(c->next), i++)
        if (i < ntop) {
            ...
            resize(c, sx, sy, sw * (c->cfact / sfacts) + ((i - ntop) < srest ? 1 : 0) - (2 * c->bw),
                   sh - (2 * c->bw), 0);
            sx += WIDTH(c) + iv;
        }
}
